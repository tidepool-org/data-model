...
Note that for some insulin pumps, even for a scheduled basal *not* interrupted by another event like a `suspend` or `temp`, the `duration` may not be the nice round number of milliseconds that might be expected given the schedule in the `pumpSettings`—e.g., 3600000 for a `basal` event lasting an hour. Because of how some pumps schedule the small pulses of insulin delivery fulfilling the scheduled `rate`, the actual duration of the `basal` may be a bit over or under the scheduled `duration`, depending on how the pulses are scheduled. 
