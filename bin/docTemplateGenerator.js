...
function getFieldSectionRegExp(field) {
  return '<!--\\sstart editable commentary on\\s' + field + '\\s-->\\s+?[\\w\\W\\s]+?<!--\\send editable commentary on\\s' + field + '\\s-->\\n';
}

var autogeneratedComment = '<!-- auto-generated doc! most areas *not* editable -->';

var doc = [autogeneratedComment + '\n'];

if (type === 'common') {
  doc.push(util.format('## %s\n', generators[type].title));
  doc.push('Jump to...\n');
  ...
  doc = doc.concat(_.flatten(allFields.map(function(field) {
    ...
    sectionBase.push(util.format('<!-- start editable commentary on %s -->', field));
    sectionBase.push('<!-- TODO -->');
    sectionBase.push(util.format('<!-- end editable commentary on %s -->\n', field));
    sectionBase.push('* * * * *\n');
    return sectionBase;
  })));
